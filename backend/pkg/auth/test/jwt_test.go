...
func newTokenHeader(headerKey string, token string) transport.Header {
...
func TestJWTServerParse(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			token: func() string {
				...
				return fmt.Sprintf(auth.BearerFormat, token)
			},
			claims: func() jwt.Claims {
	...
	next := func(ctx context.Context, req interface{}) (interface{}, error) {
		testToken, _ := auth.FromContext(ctx)
		var name string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for i := 0; i < test.goroutineNum; i++ {
				...
				go func() {
					defer wg.Done()
					ctx := transport.NewServerContext(context.Background(), &Transport{reqHeader: newTokenHeader(auth.AuthorizationKey, test.token())})
...
func TestServer(t *testing.T) {
	...
	token = fmt.Sprintf(auth.BearerFormat, token)
	...
	tests := []struct {
		...
		{
			name:          "normal",
			ctx:           transport.NewServerContext(context.Background(), &Transport{reqHeader: newTokenHeader(auth.AuthorizationKey, token)}),
		...
		},
		{
			...
			signingMethod: jwt.SigningMethodHS256,
			exceptErr:     auth.ErrMissingJwtToken,
			key:           testKey,
		},
		{
			name: "token invalid",
			ctx: transport.NewServerContext(context.Background(), &Transport{
				reqHeader: newTokenHeader(auth.AuthorizationKey, fmt.Sprintf(auth.BearerFormat, "12313123")),
			}),
			signingMethod: jwt.SigningMethodHS256,
			exceptErr:     auth.ErrTokenInvalid,
			key:           testKey,
		},
		{
			name:          "method invalid",
			...
			exceptErr:     auth.ErrUnSupportSigningMethod,
			key:           testKey,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.signingMethod != nil {
				server = auth.Server(func(token *jwt.Token) (interface{}, error) {
					return []byte(test.key), nil
				}, auth.WithSigningMethod(test.signingMethod))(next)
			} else {
				server = auth.Server(func(token *jwt.Token) (interface{}, error) {
					return []byte(test.key), nil
				})(next)
			...
			if test.exceptErr == nil {
				...
				if !ok {
					t.Errorf("except testToken is test.MapClaims, but got %T", testToken)
				}
			}
		})
	}
...
func TestClient(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err2 == nil {
				if !reflect.DeepEqual(header.Get(auth.AuthorizationKey), fmt.Sprintf(auth.BearerFormat, token)) {
					t.Errorf("except header %s, but got %s", fmt.Sprintf(auth.BearerFormat, token), header.Get(auth.AuthorizationKey))
				}
			}
		})
	}
...
// TestTokenExpire : invalid token.失效。
func TestTokenExpire(t *testing.T) {
	...
	server := auth.Server(func(token *jwt.Token) (interface{}, error) {
		return []byte(testKey), nil
	}, auth.WithSigningMethod(jwt.SigningMethodHS256))(next)
	_, err2 := server(ctx, "test expire token")
	if !errors.Is(auth.ErrTokenExpired, err2) {
		t.Errorf("except error %v, but got %v", auth.ErrTokenExpired, err2)
	}
}
...
func TestMissingKeyFunc(t *testing.T) {
	...
	test := struct {
		...
		ctx:           transport.NewServerContext(context.Background(), &Transport{reqHeader: newTokenHeader(auth.AuthorizationKey, token)}),
		signingMethod: jwt.SigningMethodHS256,
		exceptErr:     auth.ErrMissingKeyFunc,
		key:           "",
	}
...
func TestClientWithClaims(t *testing.T) {
	...
	t.Run(test.name, func(t *testing.T) {
		...
		handler := auth.Client(test.tokenProvider, auth.WithClaims(mapClaimsFunc))(next)
		header := &headerCarrier{}
		_, err2 := handler(transport.NewClientContext(context.Background(), &Transport{reqHeader: header}), "ok")
		if !errors.Is(test.expectError, err2) {
			t.Errorf("except error %v, but got %v", test.expectError, err2)
...
func TestClientWithHeader(t *testing.T) {
	...
	handler := auth.Client(tProvider, auth.WithClaims(mapClaimsFunc), auth.WithTokenHeader(tokenHeader))(next)
	header := &headerCarrier{}
	_, err2 := handler(transport.NewClientContext(context.Background(), &Transport{reqHeader: header}), "ok")
	if err2 != nil {
...
func TestNewContextAndFromContext(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			ctx := auth.NewContext(context.Background(), test.claims)
...
func TestToken(t *testing.T) {
	...
	t.Log(token)
	//
	claims, err := repo.CheckToken(context.Background(), token)
	t.Log(claims, err)
}
