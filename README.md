...
**浏览器里练真实的线上故障**。一键起一个 Linux 容器,右边是真终端,左边是任务说明,点"检查答案"后端跑判题脚本给通关反馈。

Killercoda / SadServers 的中文/国内镜像对标产品,Week 1 MVP。
...
- Go 1.21+
- Node 18+(前端),推荐 pnpm / npm 任选
- Docker Desktop(真容器 B 阶段才要,默认 mock 不需要)
- 数据库:`backend/configs/config.yaml` 已指向远程 PG / MySQL / Redis,本机不用起

### 1. 后端(默认 mock runtime,不需要 Docker)

POSIX(Linux / macOS / WSL / Git Bash):

```bash
make dev-backend
# 或 cd backend && go run ./cmd/backend -conf configs/config.yaml
```
...
```powershell
./make.ps1 install-frontend   # 首次
./make.ps1 dev-frontend
```

打开 <http://localhost:5173>,Vite 已把 `/v1 → :6039` 代理好。

### 3. 一起起

```bash
make dev          # POSIX:并行
./make.ps1 dev    # Windows:各自弹一个新 PowerShell 窗口
```

### 4. 冒烟(可选)

```bash
bash scripts/curl-smoke.sh     # POSIX,需要 jq
./scripts/curl-smoke.ps1       # Windows
```

走一遍 list → detail → start → get → check → terminate。

### 5. 切真容器(Docker)

```bash
# 1. 构建镜像
make scenarios              # POSIX
./make.ps1 scenarios        # Windows

# 2. 场景回归(可选)
make scenarios-test
./make.ps1 scenarios-test

# 3. 改 backend/configs/config.yaml:runtime.driver: mock -> docker,重启后端
```

### 6. 并发自测(C3)

浏览器开 3 个标签同时 `http://localhost:5173/s/hello-world`,每个标签自动 start 一个 attempt。
mock runtime 下三个 attempt 的 `hostPort` 应互不相同;docker runtime 下应有 3 个 `opslabs-*` 容器在跑。
重启后端后,后端日志应打 `restore running attempts from db count=N`,三个 attempt 状态保留。
...
| 需求 | POSIX | Windows |
|---|---|---|
| 起后端 | `make dev-backend` | `./make.ps1 dev-backend` |
| 起前端 | `make dev-frontend` | `./make.ps1 dev-frontend` |
| 并行起 | `make dev` | `./make.ps1 dev` |
| 装前端依赖 | `make install-frontend` | `./make.ps1 install-frontend` |
| 重新 gen | `make gen` | `./make.ps1 gen` |
| 构建场景镜像 | `make scenarios` | `./make.ps1 scenarios` |
| 场景回归 | `make scenarios-test` | `./make.ps1 scenarios-test` |
| API 冒烟 | `make smoke` | `./make.ps1 smoke` |
...
```
opslabs/
├── backend/                  Kratos v2 (Go)
│   ├── api/opslabs/v1/       Scenario / Attempt proto
│   ├── cmd/backend/          main + wire
│   ├── configs/config.yaml   DB + runtime 配置
│   └── internal/
│       ├── biz/attempt/      AttemptUsecase
│       ├── data/             AttemptRepo (GORM)
│       ├── runtime/          Runner 抽象 + mock + docker
│       ├── scenario/         场景注册表(硬编码)
│       ├── server/           kratos transport(grpc/http/reaper/bootstrapper)
│       ├── service/opslabs/  Scenario/Attempt 服务层
│       └── store/            AttemptStore 内存缓存
├── frontend/                 Vite + React + TS
│   └── src/
│       ├── pages/            Home / Scenario
│       ├── components/       Terminal / ScenarioMeta / ActionBar / PassModal
│       ├── api/              http 封装 + RQ hooks
│       └── store/            Zustand
├── scenarios/hello-world/    场景镜像源(Dockerfile + setup + check + tests)
├── scenarios-build/base/     共用基础镜像(ttyd + 排查工具)
├── scripts/                  构建 / 测试 / 冒烟(sh + ps1 双份)
├── Makefile                  POSIX 任务
└── make.ps1                  Windows 等价物
```
...
opslabs 是一个浏览器内的 Linux 排障练习平台。第一周只做一件事——**把技术路线跑通**,证明"浏览器终端 + 容器化场景 + 自动判题"这条主链路可行。
...
开发机
├── Docker Engine
│   ├── 网络: opslabs-scenarios (独立 bridge)
│   └── 容器: 多个场景实例
...
- 装 Docker,建网络 `opslabs-scenarios`
- 写基础镜像 Dockerfile(Ubuntu 22.04 + ttyd + player 用户)
- 构建 `opslabs/base:v1`
- `docker run -p 7681:7681 opslabs/base:v1`,浏览器访问能操作 bash

### Day 2:场景 1 镜像 + 后端 runtime 层

- 写场景 1 资源:welcome.txt / setup.sh / check.sh / Dockerfile
- 构建 `opslabs/hello-world:v1`,手测判题(touch flag + docker exec check.sh)
...
- 场景 1 镜像:`opslabs/hello-world:v1`
- 基础镜像:`opslabs/base:v1`
